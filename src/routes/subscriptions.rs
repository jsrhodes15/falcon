...
impl ResponseError for SubscribeError {
    fn status_code(&self) -> StatusCode {
        match self {
            ...
            SubscribeError::DatabaseError(_) | SubscribeError::StoreTokenError(_) | SubscribeError::SendEmailError(_) => StatusCode::INTERNAL_SERVER_ERROR,
        }
    }
}
...
impl std::error::Error for StoreTokenError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        // the compiler transparently cases '&sqlx::Error' into a '&dyn Error'
        Some(&self.0)
    }
}
